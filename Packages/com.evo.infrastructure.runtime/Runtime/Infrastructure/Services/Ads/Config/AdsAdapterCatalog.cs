...
namespace _Project.Scripts.Infrastructure.Services.Ads.Config
{
    ...
    public sealed class AdsAdapterCatalog : ScriptableObject, IGameConfig
    {
...
#endif
        [SerializeField] private List<AdsAdapterConfigBase> adaptersEditor = new();
        ...
        {
            AdsAdapterConfigPromptWindow.Open(this);
        }

        private void CreateAdapterConfig(Type adapterType, string typedName, bool allowDefaultName)
        {
            if (adapterType == null || !typeof(AdsAdapterConfigBase).IsAssignableFrom(adapterType))
            {
                return;
            }
            ...
            var asset = CreateInstance(adapterType) as AdsAdapterConfigBase;
            if (asset == null)
            {
                return;
            }
        ...
        private sealed class AdsAdapterConfigPromptWindow : EditorWindow
        {
            private AdsAdapterCatalog _owner;
            private string _configName = string.Empty;
            private int _selectedIndex;
            private List<Type> _adapterTypes = new();

            public static void Open(AdsAdapterCatalog owner)
            {
                if (owner == null)
                {
                    return;
                ...
                var window = CreateInstance<AdsAdapterConfigPromptWindow>();
                window._owner = owner;
                window.titleContent = new GUIContent("Create Ads Adapter Config");
            ...
            {
                _adapterTypes.Clear();
                foreach (var type in TypeCache.GetTypesDerivedFrom<AdsAdapterConfigBase>())
                {
